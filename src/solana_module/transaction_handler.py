...
class UserTransactionHandler:
    ...
    def __init__(self, private_key_str: str, compute_unit_price: int):
        """
        ...
        try:
            ...
            logger.debug(
                f"[HANDLER] Private key string starts with: {private_key_str[:20] if private_key_str else 'None'}...")
    ...
    async def sell_token(
        ...
        try:
            ...
            radium = RaydiumAmmV4(self.client.payer)
            mint = Pubkey.from_string(token_address)
            tx_signature = await radium.sell_exec(mint, sell_percentage, slippage)
            if tx_signature:
                return tx_signature
